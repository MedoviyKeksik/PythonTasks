...
def front_x(words):
    x = [i for i in words if i[0] == 'x']
    nx = [i for i in words if i[0] != 'x']
    x.sort();
    nx.sort();
    return x + nx;
#    return sorted(words, key=lambda x: '@'+x if x[0] == 'x' else x) # Но это если не используются спецсимволы)
...
def remove_adjacent(nums):
    return [nums[i] for i in xrange(len(nums)) if (i + 1 >= len(nums) or nums[i] != nums[i + 1])]
...
def main():
    print(u'Начало и конец совпадают')
    test(match_ends(['aba', 'xyz', 'aa', 'x', 'bbb']), 3)
    test(match_ends(['', 'x', 'xy', 'xyx', 'xx']), 2)
    test(match_ends(['aaa', 'be', 'abc', 'hello']), 1)

    print()
    print(u'Начинающиеся с X в начале')
    ...
    print()
    print(u'Сортировка по последнему числу')
