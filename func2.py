...
def diff():
    # first
    ans = ((1 + 100) / 2 * 100) ** 2 - sum(i * i for i in range(1, 101))
    
    #second
    lst = [i for i in range(1, 101)]
    sqrlst = map(lambda x: x * x, lst)
    ans1 = sum(lst) ** 2 - sum(sqrlst)
    return ans1
...
def max_five():
    return max(reduce(lambda x, y: int(x) * int(y), big_number[i:i+5]) for i in range(len(big_number) - 4))
...
def factorial():
    num = reduce(lambda x, y: x * y, [i for i in range(1, 101)])
